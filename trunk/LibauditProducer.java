 ...
 *
 */
public class LibauditProducer implements ProducerInterface {
    ...
    private void processFork(ArrayList<ArrayList<String>> inputFork) {
        ...
        if (sendCloningProcess == true) {
            ...
            buffer.putVertex(cloningProcess);

            //now send wcb edge
            WasControlledBy wcb = new WasControlledBy(cloningProcess, agent, "WasControlledBy", "WasControlledBy");
            buffer.putEdge(wcb);
        ...
        //send cloned Process
        buffer.putVertex(clonedProcess);

        //send cloned Edge
        //make the cloned edge
        WasTriggeredBy clone = new WasTriggeredBy(clonedProcess, cloningProcess, "WasTriggeredBy", "WasTriggeredBy");
        //send edge
        buffer.putEdge(clone);
        ...
        WasControlledBy wcb2 = new WasControlledBy(clonedProcess, agent2, "WasControlledBy", "WasControlledBy");
        buffer.putEdge(wcb2);
    ...
    private void processRead(ArrayList<ArrayList<String>> inputRead) {
        ...
        try {
            ...
            Used readEdge = new Used(readingProcess, fileRead, "Used", "Used");
    ...
    private void processWrite(ArrayList<ArrayList<String>> inputWrite) {
        ...
        try {
            ...
            int v = Integer.parseInt(fileWritten.getAnnotationValue("version"));
            v++;
            ...
            buffer.putVertex(fileWritten);

            //send the edge
            buffer.putEdge(writeEdge);
    ...
    private void processOpen(ArrayList<ArrayList<String>> inputOpen) {
        ...
        try {
            ...
            if (fileNameHasArtifact.containsKey(filePath)) {
                ...
                //add edge from process to file
                processFdIsFile.put(keyPair, fileOpened);
                //buffer.putVertex(openingProcess);
            ...
            if (OpenCloseSemanticsOn == false) {
                ...
                if (processAnnotations.get("a1").endsWith("0") || processAnnotations.get("a1").endsWith("2")) {
                    ...
                    Used u = new Used(openingProcess, fileOpened, "Used", "Used");



                    if (fileOpened.getAnnotationValue("filename").contains("libAuditPipe")) {
                        return;
                    } else if (openingProcess.getAnnotationValue("exe").endsWith("eclipse") || openingProcess.getAnnotationValue("exe").endsWith("java")) {
                        return;
                    }
    ...
    private void processExecve(ArrayList<ArrayList<String>> inputExecve) {
        ...
        if (processes.containsKey(key)) {
            ...
            //make edges
            wcb = new WasControlledBy(newProcess, agent, "WasControlledBy", "WasControlledBy");
            execve = new WasTriggeredBy(newProcess, oldProcess, "WasTriggeredBy", "WasTriggeredBy");


            //add process
            buffer.putVertex(newProcess);


            //add both edges
            buffer.putEdge(wcb);
            buffer.putEdge(execve);
            ...
            buffer.putVertex(binaryFile);
            Used u = new Used(oldProcess, binaryFile, "Used", "Used");
            buffer.putEdge(u);
            ...
            // add to our table
            processes.put(newProcess.getAnnotationValue("pid"), newProcess);
    ...
    private void processRename(ArrayList<ArrayList<String>> inputRename) {
        ...
        try {
            ...
            if (!fileNameHasArtifact.containsKey(path1)) {
                ...
                renameEdgeFileToFile = new WasDerivedFrom(newFile, renamedFile, "reanme", "rename");
                renameEdgeProcessToFile = new WasGeneratedBy(newFile, renamingProcess, "rename", "rename");

                //delete old file and all its edges. also delete that files filepath to file vertex link
                //DO THIS

                //buffer.putVertex(newFile);
    ...
    private Agent getAgent(HashMap<String, String> annotations) {
        ...
        while (agents.hasNext()) {
            ...
            while (passedAgentAnnotations.hasNext()) {
                ...
                if (storedAgent.getAnnotations().containsKey(key) && storedAgent.getAnnotationValue(key).equals(value)) {
                } else {
                    wantedAgent = false;
                    break;
                }
