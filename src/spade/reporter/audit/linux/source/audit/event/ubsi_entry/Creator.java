...
public class Creator extends spade.reporter.audit.linux.source.audit.event.creator.Creator{
	...
	public String validate(final List<Record> records){
		...
		if(!(r.getType() == spade.reporter.audit.linux.source.audit.event.record.Type.UBSI_ENTRY)){
			return "Expected UBSI_ENTRY record, got type: " + r.getType();
		}
		if(!(r instanceof spade.reporter.audit.linux.source.audit.event.record.ubsi.UbsiEntry)){
	...
	public spade.reporter.audit.linux.source.audit.event.Event create(
		...
		return new Event(
			(spade.reporter.audit.linux.source.audit.event.record.ubsi.UbsiEntry) records.get(0));
	}
}
