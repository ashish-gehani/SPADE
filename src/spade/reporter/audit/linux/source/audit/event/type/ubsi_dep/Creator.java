...
public class Creator extends spade.reporter.audit.linux.source.audit.event.creator.Creator{
	...
	public String validate(final List<Record> records){
		...
		if(!(r.getType() == spade.reporter.audit.linux.source.audit.event.record.Type.UBSI_DEP)){
			return "Expected UBSI_DEP record, got type: " + r.getType();
		}
		if(!(r instanceof spade.reporter.audit.linux.source.audit.event.record.type.ubsi.UbsiDep)){
	...
	public spade.reporter.audit.linux.source.audit.event.Event create(
		...
		return new Event(
			(spade.reporter.audit.linux.source.audit.event.record.type.ubsi.UbsiDep) records.get(0));
	}
}
