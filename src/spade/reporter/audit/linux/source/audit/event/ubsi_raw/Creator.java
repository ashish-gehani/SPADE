...
public class Creator extends spade.reporter.audit.linux.source.audit.event.creator.Creator{
	...
	public String validate(final List<Record> records){
		...
		if(!(r.getType() == spade.reporter.audit.linux.source.audit.event.record.Type.UBSI_RAW)){
			return "Expected UBSI_RAW record, got type: " + r.getType();
		}
		if(!(r instanceof spade.reporter.audit.linux.source.audit.event.record.ubsi.UbsiRaw)){
	...
	public spade.reporter.audit.linux.source.audit.event.Event create(
		...
		return new Event(
			(spade.reporter.audit.linux.source.audit.event.record.ubsi.UbsiRaw) records.get(0));
	}
}
