/*
 ...
 --------------------------------------------------------------------------------
 */
package spade.reporter.audit.linux.source.audit.event.ubsi_entry;
...
import spade.reporter.audit.linux.source.audit.event.record.ubsi.Unit;
...
public class Event extends spade.reporter.audit.linux.source.audit.event.Event{
	...
	private final spade.reporter.audit.linux.source.audit.event.record.ubsi.UbsiEntry record;

	protected Event(final spade.reporter.audit.linux.source.audit.event.record.ubsi.UbsiEntry record){
		super(record.getId(), Type.UBSI_ENTRY);
		setRecords(Arrays.asList(record));
		this.record = record;
	}

	public spade.reporter.audit.linux.source.audit.event.record.ubsi.UbsiEntry getUbsiEntryRecord(){
		return record;
	}
