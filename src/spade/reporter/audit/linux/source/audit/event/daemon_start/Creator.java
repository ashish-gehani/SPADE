...
public class Creator extends spade.reporter.audit.linux.source.audit.event.creator.Creator{
	...
	public String validate(final List<Record> records){
		...
		if(!(r instanceof spade.reporter.audit.linux.source.audit.event.record.DaemonStart)){
	...
	public spade.reporter.audit.linux.source.audit.event.Event create(
		...
		final spade.reporter.audit.linux.source.audit.event.record.DaemonStart record = (
			(spade.reporter.audit.linux.source.audit.event.record.DaemonStart) records.get(0)
		);
