...
public class Creator extends spade.reporter.audit.linux.source.audit.event.creator.Creator{
	...
	public String validate(final List<Record> records){
		...
		for(final Record r : records){
			if(r instanceof spade.reporter.audit.linux.source.audit.event.record.Syscall) count++;
		}
	...
	public spade.reporter.audit.linux.source.audit.event.Event create(
		...
		spade.reporter.audit.linux.source.audit.event.record.Syscall syscall = null;
		for(final Record r : records){
			if(r instanceof spade.reporter.audit.linux.source.audit.event.record.Syscall){
				syscall = (spade.reporter.audit.linux.source.audit.event.record.Syscall) r;
				break;
			}
		}
