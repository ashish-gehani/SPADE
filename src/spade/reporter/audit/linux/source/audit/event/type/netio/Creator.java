...
public class Creator extends spade.reporter.audit.linux.source.audit.event.Creator{
	...
	protected String validate(final List<Record> records){
		...
		if(!(r.getType() == spade.reporter.audit.linux.source.audit.event.record.Type.NETIO)){
			return "Expected NETIO record, got type: " + r.getType();
		}
		if(!(r instanceof spade.reporter.audit.linux.source.audit.event.record.type.Netio)){
	...
	protected spade.reporter.audit.linux.source.audit.event.Event create(
		...
		return new Event(
			(spade.reporter.audit.linux.source.audit.event.record.type.Netio) records.get(0));
	}
}
