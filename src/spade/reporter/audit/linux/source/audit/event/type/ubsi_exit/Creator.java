...
public class Creator extends spade.reporter.audit.linux.source.audit.event.Creator{
	...
	protected String validate(final List<Record> records){
		...
		if(!(r.getType() == spade.reporter.audit.linux.source.audit.event.record.Type.UBSI_EXIT)){
			return "Expected UBSI_EXIT record, got type: " + r.getType();
		}
		if(!(r instanceof spade.reporter.audit.linux.source.audit.event.record.type.ubsi.UbsiExit)){
	...
	protected spade.reporter.audit.linux.source.audit.event.Event create(
		...
		return new Event(
			(spade.reporter.audit.linux.source.audit.event.record.type.ubsi.UbsiExit) records.get(0));
	}
}
