...
public class VersionedID extends spade.reporter.audit.linux.platform.resource.VersionedID{
	...
	public VersionedID(
		final UnixSocketPair unixSocketPair,
		final PID pid,
		final long version
	){
		super(unixSocketPair, version);
		if(pid == null){
	...
	public VersionedID(final VersionedID other){
		this(new UnixSocketPair(other.getUnixSocketPair()), new PID(other.pid), other.getVersion());
	}

	public UnixSocketPair getUnixSocketPair(){
		return (UnixSocketPair) getResource();
	}
	...
	private int fd1(){
		return getUnixSocketPair().getFd1().getValue();
	}
