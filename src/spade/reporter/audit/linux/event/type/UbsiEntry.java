...
public class UbsiEntry extends Event{
	...
	private final spade.reporter.audit.linux.event.record.type.ubsi.UbsiEntry record;

	protected UbsiEntry(final spade.reporter.audit.linux.event.record.type.ubsi.UbsiEntry record){
		super(record.getId(), Type.UBSI_ENTRY);
		setRecords(Arrays.asList(record));
		this.record = record;
	}

	public spade.reporter.audit.linux.event.record.type.ubsi.UbsiEntry getUbsiEntryRecord(){
		return record;
	}
	...
	public static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			if(!(r.getType() == spade.reporter.audit.linux.event.record.Type.UBSI_ENTRY)){
				return "Expected UBSI_ENTRY record, got type: " + r.getType();
			}
			if(!(r instanceof spade.reporter.audit.linux.event.record.type.ubsi.UbsiEntry)){
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new UbsiEntry(
				(spade.reporter.audit.linux.event.record.type.ubsi.UbsiEntry) records.get(0));
		}
	}
}
