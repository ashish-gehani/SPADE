...
public class UbsiDep extends Event{
	...
	private final spade.reporter.audit.linux.event.record.type.ubsi.UbsiDep record;

	protected UbsiDep(final spade.reporter.audit.linux.event.record.type.ubsi.UbsiDep record){
		super(record.getId(), Type.UBSI_DEP);
		setRecords(Arrays.asList(record));
		this.record = record;
	}

	public spade.reporter.audit.linux.event.record.type.ubsi.UbsiDep getUbsiDepRecord(){
		return record;
	}
	...
	public static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			if(!(r.getType() == spade.reporter.audit.linux.event.record.Type.UBSI_DEP)){
				return "Expected UBSI_DEP record, got type: " + r.getType();
			}
			if(!(r instanceof spade.reporter.audit.linux.event.record.type.ubsi.UbsiDep)){
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new UbsiDep(
				(spade.reporter.audit.linux.event.record.type.ubsi.UbsiDep) records.get(0));
		}
	}
}
