...
public class Syscall extends Event{
	...
	private final spade.reporter.audit.linux.event.record.type.Syscall syscallRecord;

	protected Syscall(
		final spade.reporter.audit.linux.event.record.type.Syscall syscallRecord,
	...
	public spade.reporter.audit.linux.event.record.type.Syscall getSyscallRecord(){
	...
	public static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			for(final Record r : records){
				if(r instanceof spade.reporter.audit.linux.event.record.type.Syscall) count++;
			}
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			spade.reporter.audit.linux.event.record.type.Syscall syscall = null;
			for(final Record r : records){
				if(r instanceof spade.reporter.audit.linux.event.record.type.Syscall){
					syscall = (spade.reporter.audit.linux.event.record.type.Syscall) r;
					break;
				}
			}
