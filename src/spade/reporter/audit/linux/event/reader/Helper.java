 ...
 *
 * Pipeline: {@link spade.reporter.audit.linux.event.reader.LineReader} →
...
public class Helper {
    ...
    public BufferedEventReader createReader(final Config config) throws Exception {
        ...
        final spade.reporter.audit.linux.event.reader.LineReader lineReader =
            new spade.reporter.audit.linux.event.reader.Factory().create(
                config.getLineReaderType(), config
            );
        final RecordReader recordReader = new RecordReader(
            lineReader,
            new spade.reporter.audit.linux.event.record.Factory(config.isRecordFactoryVerbose())
        );
