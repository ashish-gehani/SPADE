/*
 ...
 --------------------------------------------------------------------------------
 */
package spade.reporter.audit.linux.event.record.type.ubsi;
...
import spade.reporter.audit.linux.event.record.helper.StringHelper;
...
public class UbsiDep extends Record{
	...
	public UbsiDep(
		...
		super(id, Type.UBSI_DEP, rawRecord);
	...
	public static class Creator extends Record.Creator{
		...
		public Record create(final Header header) throws MalformedRecordException{
			...
			return new UbsiDep(header.getId(), header.getRawLine());
		}
	}
}
