...
public class Netfilter extends Event{
	...
	private final spade.reporter.audit.linux.audit.event.record.Netfilter record;
	...
	public spade.reporter.audit.linux.audit.event.record.Netfilter getNetfilterRecord(){
		return record;
	}
	...
	protected static class Creator extends Event.Creator{
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new Netfilter(
				(spade.reporter.audit.linux.audit.event.record.Netfilter) records.get(0));
		}
