/*
 ...
 --------------------------------------------------------------------------------
 */
package spade.reporter.audit.linux.audit.event.type.syscall;
...
public class Creator extends spade.reporter.audit.linux.audit.event.Creator{
	...
	protected String validate(final List<Record> records){
		...
		for(final Record r : records){
			if(r instanceof spade.reporter.audit.linux.audit.event.record.type.Syscall) count++;
		}
	...
	@Override
	protected spade.reporter.audit.linux.audit.event.Event create(
		...
		spade.reporter.audit.linux.audit.event.record.type.Syscall syscall = null;
		for(final Record r : records){
			if(r instanceof spade.reporter.audit.linux.audit.event.record.type.Syscall){
				syscall = (spade.reporter.audit.linux.audit.event.record.type.Syscall) r;
				break;
			}
		}
