...
public class Netio extends Event{
	...
	private final spade.reporter.audit.linux.audit.event.record.Netio record;
	...
	public spade.reporter.audit.linux.audit.event.record.Netio getNetioRecord(){
		return record;
	}
	...
	protected static class Creator extends Event.Creator{
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new Netio(
				(spade.reporter.audit.linux.audit.event.record.Netio) records.get(0));
		}
