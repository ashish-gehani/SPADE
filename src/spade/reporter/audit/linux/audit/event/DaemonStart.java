...
public class DaemonStart extends Event{
	...
	private final spade.reporter.audit.linux.audit.event.record.DaemonStart record;
	...
	public spade.reporter.audit.linux.audit.event.record.DaemonStart getDaemonStartRecord(){
		return record;
	}
	...
	protected static class Creator extends Event.Creator{
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			final spade.reporter.audit.linux.audit.event.record.DaemonStart record = (
				(spade.reporter.audit.linux.audit.event.record.DaemonStart) records.get(0)
			);
