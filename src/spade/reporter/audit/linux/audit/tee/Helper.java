 ...
 *
 * Pipeline: {@link spade.reporter.audit.linux.audit.input.BufferedEventReader}
 * → {@link Tee} → {@link spade.reporter.audit.linux.audit.output.EventWriter}
...
public class Helper {
    ...
    public Tee createTee(final Config config) throws Exception {
        ...
        final BufferedEventReader reader =
            new spade.reporter.audit.linux.audit.input.Helper().createReader(
                config.getInputConfig()
            );
        final EventWriter writer =
            new spade.reporter.audit.linux.audit.output.Helper().createWriter(
                config.getOutputConfig()
            );
        final Factory eventFactory = new Factory(
            config.getInputConfig().isEventFactoryVerbose()
        );
        return new Tee(eventFactory, reader, writer, config.isVerbose());
    }
