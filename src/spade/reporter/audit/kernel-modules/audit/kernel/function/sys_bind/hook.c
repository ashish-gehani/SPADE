...
#include "audit/kernel/function/sys_bind/arg.h"
#include "audit/kernel/function/sys_bind/hook.h"
#include "audit/kernel/function/sys_bind/result.h"
#include "audit/util/log/log.h"


static const enum kernel_function_number global_func_num = KERN_F_NUM_SYS_BIND;
...
#define BUILD_HOOK_CONTEXT_POST(_h_ctx, _sys_res) \
    ((struct kernel_function_hook_context_post){ \
        ...
        .func_res = &(const struct kernel_function_result){ \
            .res = &(const struct kernel_function_sys_bind_result){ \
                .ret = (_sys_res) \
            }, \
            .res_size = sizeof(struct kernel_function_sys_bind_result), \
            .success = ((_sys_res) == 0) \
        } \
    })

bool kernel_function_sys_bind_hook_context_pre_is_valid(const struct kernel_function_hook_context_pre *ctx)
{
    return (
...
bool kernel_function_sys_bind_hook_context_post_is_valid(const struct kernel_function_hook_context_post *ctx)
{
    return (
...
static enum kernel_function_number kernel_function_hook_function_bind_num(void)
{
    return global_func_num;
}

static const char* kernel_function_hook_function_bind_name(void)
{
#if KERNEL_HELPER_KERNEL_PTREGS_SYSCALL_STUBS
    return "__x64_sys_bind";
#else
    return "sys_bind";
#endif
}

static void *kernel_function_hook_function_bind_original_ptr(void)
{
    return &_orig;
}

static void *kernel_function_hook_function_bind_hook(void)
{
    return _hook;
}

const struct kernel_function_hook KERNEL_FUNCTION_SYS_BIND_HOOK = {
    .get_num = kernel_function_hook_function_bind_num,
    .get_name = kernel_function_hook_function_bind_name,
    .get_orig_func_ptr = kernel_function_hook_function_bind_original_ptr,
    .get_hook_func = kernel_function_hook_function_bind_hook
};
