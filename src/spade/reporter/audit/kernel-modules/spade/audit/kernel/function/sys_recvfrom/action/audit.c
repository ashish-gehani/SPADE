...
#include "spade/audit/kernel/function/sys_recvfrom/action/audit.h"
...
static bool _is_valid_recvfrom_ctx_post(struct kernel_function_hook_context_post *ctx)
{
    return (
        kernel_function_hook_context_post_is_valid(ctx)
        && ctx->header->func_num == KERN_F_NUM_SYS_RECVFROM
...
)
{
    ...
    if (!_is_valid_recvfrom_ctx_post(ctx_post))
        return -EINVAL;
    ...
    } else
    {
        ...
        if (err != 0)
        {
            util_log_debug(log_id, "Failed helper_syscall_network_copy_saddr_and_size_from_userspace. Err: %d", err);
            return err;
        }
    }
