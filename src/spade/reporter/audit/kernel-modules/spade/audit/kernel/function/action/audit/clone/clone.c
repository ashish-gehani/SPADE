...
#include "spade/audit/kernel/function/arg/clone.h"
...
{
    return (
        sys_ctx && sys_ctx->header.type == KERNEL_FUNCTION_CONTEXT_TYPE_POST && sys_ctx->header.sys_num == __NR_clone
        && sys_ctx->header.sys_arg.arg != NULL && sys_ctx->header.sys_arg.arg_size == sizeof(struct kernel_function_arg_clone)
        && sys_ctx->sys_res.success
    );
}

int kernel_function_action_audit_clone_handle(struct kernel_function_context_post *sys_ctx)
{
    int err;
    ...
    err = helper_syscall_namespace_populate_msg(&msg, sys_ctx, NS_OP_NEW_PROCESS);
    if (err != 0)
        return err;
