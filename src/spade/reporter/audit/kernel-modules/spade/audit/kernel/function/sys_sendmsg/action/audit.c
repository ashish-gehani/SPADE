...
#include "spade/audit/kernel/function/sys_sendmsg/action/audit.h"
#include "spade/audit/kernel/function/sys_sendmsg/arg.h"
#include "spade/audit/kernel/function/sys_sendmsg/result.h"
...
static bool _is_valid_sendmsg_ctx_post(struct kernel_function_hook_context_post *ctx)
{
    return (
        kernel_function_hook_context_post_is_valid(ctx)
        && ctx->header->func_num == KERN_F_NUM_SYS_SENDMSG
...
int kernel_function_sys_sendmsg_action_audit_handle_post(
    struct kernel_function_hook_context_post *ctx_post
)
{
    const char *log_id = "kernel_function_sys_sendmsg_action_audit_handle_post";
    int err;
    ...
    struct kernel_function_sys_sendmsg_arg *sys_arg;
    struct kernel_function_sys_sendmsg_result *sys_res;

    if (!_is_valid_sendmsg_ctx_post(ctx_post))
        return -EINVAL;
    ...
    sys_arg = (struct kernel_function_sys_sendmsg_arg*)ctx_post->header->func_arg->arg;
    sys_res = (struct kernel_function_sys_sendmsg_result*)ctx_post->func_res->res;
