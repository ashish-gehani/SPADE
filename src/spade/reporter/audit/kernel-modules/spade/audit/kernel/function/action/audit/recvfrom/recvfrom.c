...
#include "spade/audit/kernel/function/arg/recvfrom.h"
...
{
    return (
        sys_ctx && sys_ctx->header.type == KERNEL_FUNCTION_CONTEXT_TYPE_POST && sys_ctx->header.sys_num == __NR_recvfrom
        && sys_ctx->header.sys_arg.arg != NULL && sys_ctx->header.sys_arg.arg_size == sizeof(struct kernel_function_arg_recvfrom)
        && sys_ctx->sys_res.success
    );
}

int kernel_function_action_audit_recvfrom_handle(struct kernel_function_context_post *sys_ctx)
{
    int err;
    ...
    struct kernel_function_arg_recvfrom *sys_arg;
    ...
    sys_arg = (struct kernel_function_arg_recvfrom*)sys_ctx->header.sys_arg.arg;
