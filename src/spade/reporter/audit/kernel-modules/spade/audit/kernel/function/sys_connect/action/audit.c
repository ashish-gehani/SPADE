...
#include "spade/audit/kernel/function/sys_connect/action/audit.h"
...
static bool _is_valid_connect_ctx_post(struct kernel_function_hook_context_post *ctx)
{
    return (
        kernel_function_hook_context_post_is_valid(ctx)
        && ctx->header->func_num == KERN_F_NUM_SYS_CONNECT
...
)
{
    ...
    if (!_is_valid_connect_ctx_post(ctx_post))
        return -EINVAL;
