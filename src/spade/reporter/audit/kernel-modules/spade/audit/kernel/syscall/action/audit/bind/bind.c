...
{
    return (
        sys_ctx && sys_ctx->header.type == SYSCALL_CONTEXT_TYPE_POST && sys_ctx->header.sys_num == __NR_bind
        && sys_ctx->header.sys_arg.arg != NULL && sys_ctx->header.sys_arg.arg_size == sizeof(struct kernel_syscall_arg_bind)
        && sys_ctx->sys_res.success
    );
}

int kernel_syscall_action_audit_bind_handle(struct kernel_syscall_context_post *sys_ctx)
{
    int err;
    ...
    sys_arg = (struct kernel_syscall_arg_bind*)sys_ctx->header.sys_arg.arg;
