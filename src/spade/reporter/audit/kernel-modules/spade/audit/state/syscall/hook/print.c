...
#include "spade/audit/state/syscall/hook/table/print.h"
...
void state_syscall_hook_write_to_seqbuf(struct seqbuf *b, const struct state_syscall_hook *state)
{
    ...
    seqbuf_print_sep(b);
    state_syscall_hook_table_write_to_seqbuf(b, &state->table);
    seqbuf_print_sep(b);
...
{
    const char *func_name = "state_syscall_hook_print";
    ...
    state_syscall_hook_write_to_seqbuf(&sb, state);
