...
public class UbsiRaw extends Event{
	...
	private final spade.reporter.audit.las.event.record.ubsi.UbsiRaw record;

	public UbsiRaw(
		final spade.reporter.audit.las.event.record.ubsi.UbsiRaw record
	){
		super(record.getEventId(), record.getTime(), Type.UBSI_RAW);
		addRecord(record);
		this.record = record;
	}

	public spade.reporter.audit.las.event.record.ubsi.UbsiRaw getUBSIRawRecord(){
		return record;
	}
	...
	protected static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			if(!(r instanceof spade.reporter.audit.las.event.record.ubsi.UbsiRaw)){
				return "Expected UBSIRaw record, got: "
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new UbsiRaw(
				(spade.reporter.audit.las.event.record.ubsi.UbsiRaw) records.get(0));
		}
	}
}
