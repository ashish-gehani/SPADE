...
public class Syscall extends Event{
	...
	private final spade.reporter.audit.las.event.record.Syscall syscallRecord;

	public Syscall(
		final spade.reporter.audit.las.event.record.Syscall syscallRecord,
	...
	public spade.reporter.audit.las.event.record.Syscall getSyscallRecord(){
	...
	protected static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			for(final Record r : records){
				if(r instanceof spade.reporter.audit.las.event.record.Syscall) count++;
			}
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			spade.reporter.audit.las.event.record.Syscall syscall = null;
			for(final Record r : records){
				if(r instanceof spade.reporter.audit.las.event.record.Syscall){
					syscall = (spade.reporter.audit.las.event.record.Syscall) r;
					break;
				}
			}
