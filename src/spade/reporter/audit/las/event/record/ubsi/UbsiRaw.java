/*
 ...
 --------------------------------------------------------------------------------
 */
package spade.reporter.audit.las.event.record.ubsi;
...
import spade.reporter.audit.las.event.record.helper.AuditStringParser;
...
import spade.reporter.audit.las.event.record.helper.ProcessInfo;
import spade.reporter.audit.las.event.record.helper.StringHelper;
...
public class UbsiRaw extends Record {
	...
	public UbsiRaw(
		...
		super(eventId, time, Type.UBSI_RAW, rawRecord);
	...
	public static class Creator extends Record.Creator{
		...
		public String validate(final Header header){
			if(header.getType() != spade.reporter.audit.las.event.record.Type.UBSI_RAW){
				return "Expected UBSI_RAW, got: " + header.getType();
			}
			if(!(header.getData().indexOf(MARKER_UBSI_RAW_RECORD) >= 0)){
				return "Expected substr '" + MARKER_UBSI_RAW_RECORD + "', got: " + header.getData();
			}
			return null;
		}
		...
		public Record create(final Header header) throws MalformedRecordException{
			...
			return new UbsiRaw(header.getEventId(), header.getTime(), header.getRawLine());
		}
