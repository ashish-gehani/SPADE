...
import spade.reporter.audit.las.event.record.Record.Creator;
import spade.reporter.audit.las.event.record.helper.Header;
import spade.reporter.audit.las.event.record.path.Path;
import spade.reporter.audit.las.event.record.ubsi.UbsiDep;
import spade.reporter.audit.las.event.record.ubsi.UbsiEntry;
import spade.reporter.audit.las.event.record.ubsi.UbsiExit;
import spade.reporter.audit.las.event.record.ubsi.UbsiRaw;

/**
 * Factory class for creating Record subclass instances from raw audit log lines.
 *
 * Iterates a fixed, ordered list of {@link Record.Creator} instances, calling
 * {@link Record.Creator#matches} on each until one accepts the raw line,
 * then delegates to {@link Record.Creator#create}.
 *
 * Returns null for record types that should be skipped (EOE, PROCTITLE, UNKNOWN)
 * or when no creator matches.
 */
