...
public class Netfilter extends Record {
	...
	public static class Creator extends Record.Creator{
		...
		public String validate(final Header header){
			if(header.getType() != spade.reporter.audit.las.event.record.Type.NETFILTER){
				return "Expected NETFILTER, got: " + header.getType();
			}
			if(!(header.getData().indexOf(MARKER_NF_SUBTYPE) >= 0)){
				return "Expected substr '" + MARKER_NF_SUBTYPE + "', got: " + header.getData();
			}
			return null;
		}
