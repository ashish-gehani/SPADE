...
public class Netio extends Record{
	...
	public static class Creator extends Record.Creator{
		...
		public String validate(final Header header){
			if(header.getType() != spade.reporter.audit.las.event.record.Type.NETIO){
				return "Expected NETIO, got: " + header.getType();
			}
			if(!(header.getData().indexOf(MARKER_NETIO_RECORD) >= 0)){
				return "Expected substr '" + MARKER_NETIO_RECORD + "', got: " + header.getData();
			}
			return null;
		}
