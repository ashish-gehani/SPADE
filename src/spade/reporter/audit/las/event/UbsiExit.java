...
public class UbsiExit extends Event{
	...
	private final spade.reporter.audit.las.event.record.ubsi.UbsiExit record;

	public UbsiExit(final spade.reporter.audit.las.event.record.ubsi.UbsiExit record){
		super(record.getEventId(), record.getTime(), Type.UBSI_EXIT);
		addRecord(record);
		this.record = record;
	}

	public spade.reporter.audit.las.event.record.ubsi.UbsiExit getUbsiExitRecord(){
		return record;
	}
	...
	protected static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			if(!(r instanceof spade.reporter.audit.las.event.record.ubsi.UbsiExit)){
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new UbsiExit((spade.reporter.audit.las.event.record.ubsi.UbsiExit) records.get(0));
		}
	}
}
