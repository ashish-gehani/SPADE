...
public class Netfilter extends Event{
	...
	private final spade.reporter.audit.las.event.record.Netfilter record;

	public Netfilter(
		final spade.reporter.audit.las.event.record.Netfilter record
	){
	...
	public spade.reporter.audit.las.event.record.Netfilter getNetfilterRecord(){
		return record;
	}
	...
	protected static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			if(!(r instanceof spade.reporter.audit.las.event.record.Netfilter)){
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new Netfilter(
				(spade.reporter.audit.las.event.record.Netfilter) records.get(0));
		}
