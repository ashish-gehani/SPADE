...
public class DaemonStart extends Event{
	...
	private final spade.reporter.audit.las.event.record.DaemonStart record;

	public DaemonStart(
		final spade.reporter.audit.las.event.record.DaemonStart record
	){
	...
	public spade.reporter.audit.las.event.record.DaemonStart getDaemonStartRecord(){
		return record;
	}
	...
	protected static class Creator extends Event.Creator{
		...
		protected String validate(final List<Record> records){
			...
			if(!(r instanceof spade.reporter.audit.las.event.record.DaemonStart)){
		...
		protected Event create(final List<Record> records) throws MalformedEventException{
			...
			return new DaemonStart(
				(spade.reporter.audit.las.event.record.DaemonStart) records.get(0));
		}
	}
}