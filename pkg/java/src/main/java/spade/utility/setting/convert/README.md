...
Each numeric type has its own class in the `spade.utility.setting.convert.numbers` package: `Ints` (for `int`), `Longs` (for `long`), and `Doubles` (for `double`). Each exposes the same shape: `get(setting, key)` / `get(setting, key, min, max)`, `opt(setting, key)` / `opt(setting, key, min, max)` / `opt(setting, key, min, max, defaultValue)`, and `parse(String, min, max)` for parsing directly on a raw string, without a `Setting` lookup.
