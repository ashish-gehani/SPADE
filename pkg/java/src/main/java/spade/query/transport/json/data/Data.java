...
public abstract class Data {
    ...
    public static Data fromJSON(final ObjectNode node) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Unknown data type: " + typeNode.asText(), e);
        }

        switch (type) {
            case ERROR:
                return spade.query.transport.json.data.error.Error.fromJSON(node);
            case GRAPH:
                return spade.query.transport.json.data.graph.Graph.fromJSON(node);
            case TEXT:
                return spade.query.transport.json.data.text.Text.fromJSON(node);
            default:
