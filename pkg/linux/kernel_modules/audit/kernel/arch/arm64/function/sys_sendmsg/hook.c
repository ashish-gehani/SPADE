...
#define BUILD_HOOK_CONTEXT(_sockfd, _msg, _flags) \
	...
	static asmlinkage long _hook(const struct pt_regs *regs)
    {
        ...
        kernel_arch_common_function_sys_sendmsg_hook_pre(&h_ctx);
		...
		kernel_arch_common_function_sys_sendmsg_hook_post(&h_ctx, res);
		return res;
	}
...
{
#if KERNEL_HELPER_KERNEL_PTREGS_SYSCALL_STUBS
    return "__arm64_sys_sendmsg";
#else
    return "sys_sendmsg";
#endif
}

static void *kernel_function_hook_function_sendmsg_original_ptr(void)
{
    return &_orig;
}

static void *kernel_function_hook_function_sendmsg_hook(void)
{
    return _hook;
}

static const struct kernel_function_hook KERNEL_FUNCTION_SYS_SENDMSG_HOOK = {
    .get_num = kernel_arch_common_function_hook_function_sendmsg_num,
