...
#define BUILD_HOOK_CONTEXT(_fd, _addr, _addrlen) \
	...
	static asmlinkage long _hook(const struct pt_regs *regs)
    {
        ...
        uint32_t addr_size = (uint32_t)(regs->regs[2]);
		...
		kernel_arch_common_function_sys_bind_hook_post(&h_ctx, res);
		return res;
	}
...
static const struct kernel_function_hook KERNEL_FUNCTION_SYS_BIND_HOOK = {
    .get_num = kernel_arch_common_function_hook_function_bind_num,
...
const struct kernel_function_hook* kernel_arch_common_overridable_function_sys_bind_hook_get(void)
{
    return &KERNEL_FUNCTION_SYS_BIND_HOOK;
}
