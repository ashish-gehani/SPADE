...
)
{
    ...
    case KERN_F_NUM_SYS_UNSHARE:
        *dst = __NR_unshare;
        break;
    /* aarch64's native 64-bit ABI has no SYS_fork / SYS_vfork syscall numbers (glibc's
     * fork()/vfork() are implemented via clone() on this architecture), so KERN_F_NUM_SYS_FORK
     * and KERN_F_NUM_SYS_VFORK fall through to the default case below. */
    default:
