...
#define BUILD_HOOK_CONTEXT(_flags) \
	...
	static asmlinkage long _hook(const struct pt_regs *regs)
    {
		...
		kernel_arch_common_function_sys_unshare_hook_post(&h_ctx, res);
		return res;
	}
...
static const struct kernel_function_hook KERNEL_FUNCTION_SYS_UNSHARE_HOOK = {
    .get_num = kernel_arch_common_function_hook_function_unshare_num,
...
{
    return &KERNEL_FUNCTION_SYS_UNSHARE_HOOK;
}
