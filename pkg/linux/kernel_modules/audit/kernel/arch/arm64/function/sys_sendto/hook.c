...
#define BUILD_HOOK_CONTEXT(_sockfd, _buf, _len, _flags, _dest_addr, _addrlen) \
	...
	static asmlinkage long _hook(const struct pt_regs *regs)
    {
        ...
        const void __user *buf = (const void __user *)(regs->regs[1]);
        ...
        kernel_arch_common_function_sys_sendto_hook_post(&h_ctx, res);
		return res;
	}
...
static const struct kernel_function_hook KERNEL_FUNCTION_SYS_SENDTO_HOOK = {
    .get_num = kernel_arch_common_function_hook_function_sendto_num,
...
{
    return &KERNEL_FUNCTION_SYS_SENDTO_HOOK;
}
