...
#include "audit/kernel/arch/common/function/sys_accept/arg.h"
#include "audit/kernel/arch/common/function/sys_accept/hook.h"
#include "audit/util/log/log.h"


#define BUILD_HOOK_CONTEXT(_fd, _addr, _addrlen) \
    ((const struct kernel_function_hook_context){ \
        .func_num = kernel_arch_common_function_hook_function_accept_num(), \
        .func_arg = &(const struct kernel_function_arg){ \
            .arg = &(const struct kernel_function_sys_accept_arg){ \
                .sockfd = (_fd), \
                .addr = (_addr), \
                .addrlen = (_addrlen) \
            }, \
	...
	static asmlinkage long _hook(const struct pt_regs *regs)
    {
        ...
        struct sockaddr __user *addr = (struct sockaddr *)(regs->regs[1]);
        uint32_t __user *addr_size = (uint32_t *)(regs->regs[2]);

        const struct kernel_function_hook_context h_ctx = BUILD_HOOK_CONTEXT(fd, addr, addr_size);

        kernel_arch_common_function_sys_accept_hook_pre(&h_ctx);
		...
		kernel_arch_common_function_sys_accept_hook_post(&h_ctx, res);
		return res;
	}
...
static const char* kernel_function_hook_function_accept_name(void)
{
#if KERNEL_HELPER_KERNEL_PTREGS_SYSCALL_STUBS
    return "__arm64_sys_accept";
#else
    return "sys_accept";
#endif
}

static void *kernel_function_hook_function_accept_original_ptr(void)
{
    return &_orig;
}

static void *kernel_function_hook_function_accept_hook(void)
{
    return _hook;
}

static const struct kernel_function_hook KERNEL_FUNCTION_SYS_ACCEPT_HOOK = {
    .get_num = kernel_arch_common_function_hook_function_accept_num,
    .get_name = kernel_function_hook_function_accept_name,
    .get_orig_func_ptr = kernel_function_hook_function_accept_original_ptr,
    .get_hook_func = kernel_function_hook_function_accept_hook
};

const struct kernel_function_hook* kernel_arch_common_overridable_function_sys_accept_hook_get(void)
{
    return &KERNEL_FUNCTION_SYS_ACCEPT_HOOK;
}
