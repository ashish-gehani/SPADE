...
#define BUILD_HOOK_CONTEXT(_fd, _addr, _addrlen) \
	...
	static asmlinkage long _hook(const struct pt_regs *regs)
    {
        ...
        kernel_function_sys_accept_hook_pre(&h_ctx);
		...
		kernel_function_sys_accept_hook_post(&h_ctx, res);
		return res;
	}
...
const struct kernel_function_hook* kernel_function_sys_accept_hook_get(void)
{
