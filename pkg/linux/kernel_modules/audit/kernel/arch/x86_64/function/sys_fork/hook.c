...
#define BUILD_HOOK_CONTEXT() \
	...
	static asmlinkage long _hook(const struct pt_regs *regs)
    {
		const char *log_id = "sys_fork::_hook";
		long res;

		const struct kernel_function_hook_context h_ctx = BUILD_HOOK_CONTEXT();

		kernel_arch_common_function_sys_fork_hook_pre(&h_ctx);
		...
		kernel_arch_common_function_sys_fork_hook_post(&h_ctx, res);
		return res;
	}
...
static const struct kernel_function_hook KERNEL_FUNCTION_SYS_FORK_HOOK = {
    .get_num = kernel_arch_common_function_hook_function_fork_num,
...
{
    return &KERNEL_FUNCTION_SYS_FORK_HOOK;
}
