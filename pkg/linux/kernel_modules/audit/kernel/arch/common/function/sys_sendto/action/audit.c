...
#include "audit/kernel/arch/common/function/sys_sendto/action/audit.h"
#include "audit/kernel/arch/common/function/sys_sendto/arg.h"
#include "audit/kernel/arch/common/function/sys_sendto/hook.h"
#include "audit/kernel/arch/common/function/sys_sendto/result.h"
#include "audit/msg/network/network.h"
#include "audit/msg/ops.h"
#include "audit/kernel/arch/common/helper/network.h"
#include "audit/kernel/arch/common/helper/audit_log.h"
...
int kernel_arch_common_function_sys_sendto_action_audit_handle_post(
    const struct kernel_function_hook_context_post *ctx_post
)
{
    const char *log_id = "kernel_arch_common_function_sys_sendto_action_audit_handle_post";
    int err;
    ...
    struct kernel_function_sys_sendto_arg *sys_arg;
    struct kernel_function_sys_sendto_result *sys_res;

    if (!kernel_arch_common_function_sys_sendto_hook_context_post_is_valid(ctx_post))
        return -EINVAL;
    ...
    sys_arg = (struct kernel_function_sys_sendto_arg*)ctx_post->header->func_arg->arg;
    sys_res = (struct kernel_function_sys_sendto_result*)ctx_post->func_res->res;
    ...
    {
        err = kernel_arch_common_helper_network_copy_only_saddr_from_userspace(
            &remote_saddr, &remote_saddr_size,
            sys_arg->dest_addr, sys_arg->addrlen
        );
        if (err != 0)
        {
            util_log_debug(log_id, "Failed kernel_arch_common_helper_network_copy_only_saddr_from_userspace. Err: %d", err);
            return err;
        }
    }
