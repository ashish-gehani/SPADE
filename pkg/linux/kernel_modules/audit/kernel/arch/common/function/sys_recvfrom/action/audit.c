...
#include "audit/kernel/arch/common/function/sys_recvfrom/action/audit.h"
...
int kernel_arch_common_function_sys_recvfrom_action_audit_handle_post(
    const struct kernel_function_hook_context_post *ctx_post
)
{
    const char *log_id = "kernel_arch_common_function_sys_recvfrom_action_audit_handle_post";
    int err;
    ...
    if (!kernel_arch_common_function_sys_recvfrom_hook_context_post_is_valid(ctx_post))
        return -EINVAL;
    ...
    } else
    {
        ...
        if (err != 0)
        {
            util_log_debug(log_id, "Failed kernel_arch_common_helper_network_copy_saddr_and_size_from_userspace. Err: %d", err);
            return err;
        }
    }
