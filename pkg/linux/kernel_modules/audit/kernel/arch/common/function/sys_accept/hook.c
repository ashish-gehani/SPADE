...
#include "audit/kernel/arch/common/function/sys_accept/result.h"
...
#define BUILD_HOOK_CONTEXT_POST(_h_ctx, _sys_res) \
    ((struct kernel_function_hook_context_post){ \
        ...
        .func_res = &(const struct kernel_function_result){ \
            .res = &(const struct kernel_function_sys_accept_result){ \
                .ret = (_sys_res) \
            }, \
            .res_size = sizeof(struct kernel_function_sys_accept_result), \
            .success = ((_sys_res) >= 0) \
        } \
    })
...
/* Arch-specific implementations (e.g. audit/kernel/arch/x86_64/function/sys_accept/hook.c) provide
...
const struct kernel_function_hook* __weak kernel_arch_common_overridable_function_sys_accept_hook_get(void)
{
    return NULL;
}

bool kernel_arch_common_function_sys_accept_hook_context_pre_is_valid(const struct kernel_function_hook_context_pre *ctx)
{
    return (
...
bool kernel_arch_common_function_sys_accept_hook_context_post_is_valid(const struct kernel_function_hook_context_post *ctx)
{
    return (
