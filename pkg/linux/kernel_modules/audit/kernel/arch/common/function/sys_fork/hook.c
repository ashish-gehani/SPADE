...
#include "audit/kernel/arch/common/function/sys_fork/hook.h"
#include "audit/kernel/arch/common/function/sys_fork/arg.h"
#include "audit/kernel/arch/common/function/sys_fork/result.h"
...
/* Arch-specific implementations (e.g. audit/kernel/arch/x86_64/function/sys_fork/hook.c) provide
...
const struct kernel_function_hook* __weak kernel_arch_common_overridable_function_sys_fork_hook_get(void)
{
    return NULL;
}

bool kernel_arch_common_function_sys_fork_hook_context_pre_is_valid(const struct kernel_function_hook_context_pre *ctx)
{
    return (
        kernel_arch_common_function_hook_context_pre_is_valid(ctx)
        && ctx->header->func_num == global_func_num
        && ctx->header->func_arg->arg_size == sizeof(struct kernel_function_sys_fork_arg)
    );
}

bool kernel_arch_common_function_sys_fork_hook_context_post_is_valid(const struct kernel_function_hook_context_post *ctx)
{
    return (
        kernel_arch_common_function_hook_context_post_is_valid(ctx)
        && ctx->header->func_num == global_func_num
        && ctx->header->func_arg->arg_size == sizeof(struct kernel_function_sys_fork_arg)
        && ctx->func_res->res_size == sizeof(struct kernel_function_sys_fork_result)
