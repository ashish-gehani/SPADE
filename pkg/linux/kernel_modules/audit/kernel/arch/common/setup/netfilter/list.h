...
#include "audit/kernel/arch/common/netfilter/netfilter.h"


const struct nf_hook_ops kernel_arch_common_netfilter_setup_list_hooks[] = {
	{
		.hook = kernel_arch_common_netfilter_hook_first, .pf = NFPROTO_IPV4,
		.hooknum = NF_INET_LOCAL_OUT, .priority = NF_IP_PRI_FIRST
	},
	{
		.hook = kernel_arch_common_netfilter_hook_last, .pf = NFPROTO_IPV4,
		.hooknum = NF_INET_LOCAL_OUT, .priority = NF_IP_PRI_LAST
	},
	{
		.hook = kernel_arch_common_netfilter_hook_first, .pf = NFPROTO_IPV6,
		.hooknum = NF_INET_LOCAL_OUT, .priority = NF_IP_PRI_FIRST
	},
	{
		.hook = kernel_arch_common_netfilter_hook_last, .pf = NFPROTO_IPV6,
		.hooknum = NF_INET_LOCAL_OUT, .priority = NF_IP_PRI_LAST
	},
///
	{
...
const int kernel_arch_common_netfilter_setup_list_hooks_size = sizeof(kernel_arch_common_netfilter_setup_list_hooks) / sizeof(kernel_arch_common_netfilter_setup_list_hooks[0]);
