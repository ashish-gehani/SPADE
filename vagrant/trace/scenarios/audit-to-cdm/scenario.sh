...
function scenario_description() {
    echo "Replay an auditd log into CDM storage"
}
...
function scenario_install_config() {
    ...
    cp "${ENV_SCENARIO_SELECTED_HOME}/cfg/spade.storage.CDM.config" "${ENV_SPADE_HOME}/cfg/"
    cp "${ENV_SCENARIO_SELECTED_HOME}/cfg/spade.reporter.Audit.config" "${ENV_SPADE_HOME}/cfg/"

    output_file="$(grep -oP '^kafka\.output\.file=\K.*' "${ENV_SPADE_HOME}/cfg/spade.storage.CDM.config")"
...
function scenario_execute() {
    spade_run_control_command "add storage CDM"
    ...
    spade_run_control_command "remove storage CDM"
}

function scenario_post_execution_work() {
    local installed_storage_config="${ENV_SPADE_HOME}/cfg/spade.storage.CDM.config"
    local topic
    local consumed_file="/tmp/trace/audit-to-cdm/consumed-topic.txt"
    local output_file
