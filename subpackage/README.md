...
This directory contains SPADE's subpackages. General guidelines for a subpackage:

## Structure

- Each subpackage is a standalone autoconf/automake package (`AC_INIT`, version `2.0`), using the shared `build-aux`/`m4` dirs from further up the tree, with `AM_MAINTAINER_MODE([disable])`.
- Sources live under `src/`, kept self-contained per subpackage (a symlink to `subpackage/shared/` when the code is shared between subpackages), for self-contained subpackages and easier development environment configuration.
- Each subpackage has its own `README.md` documenting itself and noting any deviations from these guidelines.
...
- Each platform (e.g. `linux/`, `mac/`) has its own `configure.ac`/`Makefile.am` that groups multiple subpackages together. This is where individual subpackages are enabled or disabled by default (e.g. via `AC_ARG_ENABLE`) before being configured as subdirs.
