...
`CLI` is a `Tool` that connects to a running SPADE instance over a host/port connection and exchanges lines with it, via `Connection`. This is a general shape, not tied to any one SPADE-side listener — SPADE's query client and control client are both examples of the kind of connection this can front.
