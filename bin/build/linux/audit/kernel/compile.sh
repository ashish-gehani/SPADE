...
# globals
KERNEL_MODULES_DEBUG=""
KERNEL_MODULES_SRC=""
KERNEL_MODULES_INSTALL=""


print_help() {
    echo "Usage: $(basename "$0") --kernel_modules_src <path> --kernel_modules_install <path> --kernel_modules_debug <true|false>"
    echo ""
    echo "Options:"
    ...
    echo "    --kernel_modules_debug <true|false> Enable debug build (default: false)"
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --kernel_modules_debug)   KERNEL_MODULES_DEBUG="$2";   shift 2 ;;
            --help)                   print_help ;;
...
validate_args() {
    ...
    if [[ -z "${KERNEL_MODULES_DEBUG}" ]]; then
        echo "Error: --kernel_modules_debug is required"
        exit 1
    fi
}
