...
- `cfg/spade.utility.mcp.server.tool.registry.Registry/**` — MCP tool definitions (`spade_control`, `spade_query`, `spade_control_doc`, `spade_query_doc`, ...)
...
- `pkg/java/src/test/java/spade/utility/mcp/**` — the MCP client and server Java tests
...
- A new test directory under `spade.utility.mcp` is already covered by the `pkg/java/src/test/java/spade/utility/mcp/**` wildcard — no change needed.
...
- A new MCP tool definition under `cfg/spade.utility.mcp.server.tool.registry.Registry/` is already covered by `cfg/spade.utility.mcp.server.tool.registry.Registry/**` — no change needed.
...
When in doubt, prefer adding a path over relying on a broader wildcard — this filter's job is precision (skip irrelevant runs), not recall (catch every conceivable dependency); an unfiltered build+test still runs on every push regardless.
